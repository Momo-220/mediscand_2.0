...
export default function ResultatAnalyse({ resultat, imageData, onReset }: ResultatAnalyseProps) {
  ...
  return (
    <motion.div 
      ...
      exit={{ opacity: 0, y: 20 }}
      className="w-full max-w-4xl mx-auto bg-gray-800 rounded-xl shadow-2xl border border-gray-700 overflow-hidden"
    >
      <div className="sticky top-0 z-10 bg-gradient-to-r from-blue-900 to-indigo-900 px-6 py-4 border-b border-gray-700 flex justify-between items-center">
        <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-blue-200">
          Résultat de l'analyse
        </h2>
        <button
          onClick={onReset}
          className="flex items-center justify-center bg-gray-800/30 hover:bg-gray-700/50 text-white rounded-full w-10 h-10 transition-colors"
        >
      ...
      <div className="overflow-y-auto max-h-[calc(100vh-12rem)]">
        {resultat ? (
          <div className="flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-1/3 bg-gray-900 p-6 flex flex-col items-center justify-center">
              {imageData ? (
                <img 
                  src={imageData} 
                  alt="Médicament analysé" 
                  className="w-full max-w-[240px] h-auto object-contain rounded-lg shadow-lg border border-gray-700"
                />
              ) : (
              ...
              <div className="mt-6 w-full">
                <h3 className="text-xl font-bold text-white mb-2">{resultat.detailsAnalyse?.nomCommercial}</h3>
                
                <div className="flex flex-wrap gap-2 mb-4">
                  {resultat.detailsAnalyse?.laboratoire && (
                    <span className="px-3 py-1 bg-blue-900/40 text-blue-300 text-xs rounded-full">
                      {resultat.detailsAnalyse.laboratoire}
                    </span>
                  )}
                  {resultat.detailsAnalyse?.dosage && (
                    <span className="px-3 py-1 bg-green-900/40 text-green-300 text-xs rounded-full">
                      {resultat.detailsAnalyse.dosage}
                    </span>
                  )}
                </div>
                ...
                {resultat.detailsAnalyse?.laboratoire && (
                  <div className="mb-4 p-3 bg-gray-800/80 border border-gray-700 rounded-lg">
                    <p className="text-sm text-gray-400 mb-1">Laboratoire</p>
                    <p className="text-white font-medium">{resultat.detailsAnalyse.laboratoire}</p>
                  </div>
                )}

                {resultat.detailsAnalyse?.dci && (
                  <div className="p-3 bg-gray-800/80 border border-gray-700 rounded-lg">
                    <p className="text-sm text-gray-400 mb-1">DCI / Principe actif</p>
                    <p className="text-white font-medium">{resultat.detailsAnalyse.dci}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="w-full md:w-2/3 p-6">
              <div className="mb-6">
                ...
                <p className="text-gray-300 whitespace-pre-wrap">{resultat.description}</p>
              </div>
              ...
              {sections.map((section, index) => (
                <div key={index} className="mb-6">
                  {section.items.filter(item => item.value && item.value.trim() !== "").length > 0 && (
                    <>
                      <h3 className="text-lg font-bold text-white mb-3 pb-2 border-b border-gray-700 flex items-center">
                        <span className="mr-2 text-blue-400">{section.titre}</span>
                      </h3>
              ...
              {sections.map((section, index) => (
                section.items.filter(item => item.value && item.value.trim() !== "").length > 0 && (
                  <div key={`${section.titre}-additional`} className="mt-8 pt-4 border-t border-gray-700">
